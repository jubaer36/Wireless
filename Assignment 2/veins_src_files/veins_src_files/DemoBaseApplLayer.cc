...
std::ofstream DemoBaseApplLayer::logFile;  // Added later by S. M. Sabit Bananee
bool DemoBaseApplLayer::logFileInitialized = false; // Added later by S. M. Sabit Bananee

std::ofstream DemoBaseApplLayer::logFileNumCurrentVehicles;  // Added later by S. M. Sabit Bananee
bool DemoBaseApplLayer::logFileInitializedNumCurrentVehicles = false; // Added later by S. M. Sabit Bananee
////////////////////////////////////////////////////////
...
void DemoBaseApplLayer::initialize(int stage)
{
    ...
    if (!logFileInitialized) {
        logFile.open("veinsApplLayerMessageInfo.csv", std::ios::out);  // Overwrite each run
        logFile << "Receiver,Sender,Packet Type,Delay\n";
        logFileInitialized = true;
    }

    if (!logFileInitializedNumCurrentVehicles) {
        logFileNumCurrentVehicles.open("veinsCurrentVehiclesInfo.csv", std::ios::out);  // Overwrite each run
        logFileNumCurrentVehicles << "Simulation Time,Sender,Packet Type,Number of Vehicles\n";
        logFileInitializedNumCurrentVehicles = true;
   }
    //////////////////////////////////////
...
void DemoBaseApplLayer::populateWSM(BaseFrame1609_4* wsm, LAddress::L2Type rcvId, int serial)
{
    ...
    if (DemoSafetyMessage* bsm = dynamic_cast<DemoSafetyMessage*>(wsm)) {
        ...
        bsm->setSenderAddress(myId); // Added later by S. M. Sabit Bananee
        ...
        wsa->setSenderAddress(myId); // Added later by S. M. Sabit Bananee
    }
    else {
        if (dataOnSch)
...
void DemoBaseApplLayer::handleLowerMsg(cMessage* msg)
{
    ...
    // Added later by S. M. Sabit Bananee
    simtime_t creationTime = wsm->getCreationTime();
    ...
    if (DemoSafetyMessage* bsm = dynamic_cast<DemoSafetyMessage*>(wsm)) {
        receivedBSMs++;
        packetType = "bsm"; // Added Later by S. M. Sabit Bananee
        senderAddress = bsm->getSenderAddress(); // Added Later by S. M. Sabit Bananee

        onBSM(bsm);
    }
    else if (DemoServiceAdvertisment* wsa = dynamic_cast<DemoServiceAdvertisment*>(wsm)) {
        receivedWSAs++;
        packetType = "wsa"; // Added Later by S. M. Sabit Bananee
        senderAddress = wsa->getSenderAddress(); // Added Later by S. M. Sabit Bananee
        onWSA(wsa);
    }
    else {
        receivedWSMs++;
        // Added Later by S. M. Sabit Bananee
        packetType = "wsm";
        if (TraCIDemo11pMessage* demo11pMsg = dynamic_cast<TraCIDemo11pMessage*>(wsm)){
                    senderAddress = demo11pMsg->getSenderAddress();
                    packetType = "TraCIDemo11pMessage";
        }
        ///////////////////////////////////////
        onWSM(wsm);
    }
    if(senderAddress != -1) logFile << getParentModule()->getFullName() << "," << getSimulation()->getModule(senderAddress)->getParentModule()->getFullName()  << "," << packetType << "," << delay << std::endl;
    else logFile << getParentModule()->getFullName() << "," << senderAddress << "," << packetType << "," << delay << std::endl;
...
void DemoBaseApplLayer::finish()
{
    ...
    if (logFileNumCurrentVehicles.is_open()) logFileNumCurrentVehicles.close(); // Added later by S. M. Sabit Bananee
...
void DemoBaseApplLayer::sendDown(cMessage* msg)
{
    ...
    if(manager){
        auto vehicles = manager->getManagedHosts();
        ...
        if (DemoSafetyMessage* bsm = dynamic_cast<DemoSafetyMessage*>(wsm)) packetType = "bsm";
        else if (DemoServiceAdvertisment* wsa = dynamic_cast<DemoServiceAdvertisment*>(wsm)) packetType = "wsa";
        else {
            packetType = "wsm";
            if (TraCIDemo11pMessage* demo11pMsg = dynamic_cast<TraCIDemo11pMessage*>(wsm)) packetType = "TraCIDemo11pMessage";
        }

        logFileNumCurrentVehicles << simTime() << "," << getParentModule()->getFullName() << "," << packetType << "," << numVehicles << std::endl;
    }
    ///////////////////////////////////////////////////
