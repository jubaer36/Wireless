...
std::ofstream Mac1609_4::logFile;  // Added later by S. M. Sabit Bananee
bool Mac1609_4::logFileInitialized = false; // Added later by S. M. Sabit Bananee
std::ofstream Mac1609_4::logFileChannelAccessDelay;  // Added later by S. M. Sabit Bananee
bool Mac1609_4::logFileInitializedChannelAccessDelay = false; // Added later by S. M. Sabit Bananee
...
void Mac1609_4::initialize(int stage)
{
    ...
    //Added later by S. M. Sabit Bananee
    if (!logFileInitialized) {
        logFile.open("veinsMacLayerMessageInfo.csv", std::ios::out);  // Overwrite each run
        logFile << "Sender,Destination Address,Receiver,Packet Type,Clock Time at Reception Event\n";
        logFileInitialized = true;
    }
    if (!logFileInitializedChannelAccessDelay) {
        logFileChannelAccessDelay.open("veinsChannelAccessDelayInfo.csv", std::ios::out);  // Overwrite each run
        logFileChannelAccessDelay << "Sending Node,Packet Type, Channel Access Delay\n";
        logFileInitializedChannelAccessDelay = true;
    }
    //////////////////////////////////////
...
void Mac1609_4::handleSelfMsg(cMessage* msg)
{
    ...
    if (msg == nextChannelSwitch) {
        ...
        mac->setReceiveFromUpperTime(pktToSend->getReceiveFromUpperTime());
        //mac->setReceiveFromUpperTime(simTime()); // Added later by S. M. Sabit Bananee
...
void Mac1609_4::handleUpperMsg(cMessage* msg)
{
    ...
    thisMsg->setReceiveFromUpperTime(simTime()); // Added later by S. M. Sabit Bananee
...
void Mac1609_4::finish()
{
    ...
    if (logFile.is_open()) logFile.close(); // Added later by S. M. Sabit Bananee
    if (logFileChannelAccessDelay.is_open()) logFileChannelAccessDelay.close(); // Added later by S. M. Sabit Bananee
...
void Mac1609_4::sendFrame(Mac80211Pkt* frame, simtime_t delay, Channel channelNr, MCS mcs, double txPower_mW)
{
    ...
    logFileChannelAccessDelay << myId << ", " << frame->getPacketType() <<"," << simTime() - frame->getReceiveFromUpperTime()  << std::endl; // Added later by S. M. Sabit Bananee
...
//    unique_ptr<BaseFrame1609_4> wsm_u(check_and_cast<BaseFrame1609_4*>(macPkt->decapsulate()));
//    auto ctrlInfo_u = new PhyToMacControlInfo(res);
//    ctrlInfo_u->setSourceAddress(macPkt->getSrcAddr());
//    wsm_u->setControlInfo(ctrlInfo_u);
////    BaseFrame1609_4* wsm = dynamic_cast<BaseFrame1609_4*>(msg);
////    EV << wsm_u << std:: endl;
////    throw cRuntimeError ("halted\n");
////
//    if (DemoSafetyMessage* bsm = dynamic_cast<DemoSafetyMessage*>(wsm_u.get())) packetType = "bsm";
//    else if (DemoServiceAdvertisment* wsa = dynamic_cast<DemoServiceAdvertisment*>(wsm_u.get())) packetType = "wsa";
//    else if (TraCIDemo11pMessage* demo11pMsg = dynamic_cast<TraCIDemo11pMessage*>(wsm_u.get())) packetType = "TraCIDemo11pMessage";
//    else  packetType = "wsm";

    if (macPkt->getDestAddr() != LAddress::L2BROADCAST())
        logFile << getSimulation()->getModule(macPkt->getSrcAddr())->getParentModule()->getFullName() << "," << getSimulation()->getModule(macPkt->getDestAddr())->getParentModule()->getFullName() << "," << getSimulation()->getModule(myMacAddr)->getParentModule()->getFullName() << "," << packetType << "," << simTime()  << std::endl; // Added later by S. M. Sabit Bananee
    else
        logFile << getSimulation()->getModule(macPkt->getSrcAddr())->getParentModule()->getFullName() << "," << "BroadCast" << "," << getSimulation()->getModule(myMacAddr)->getParentModule()->getFullName() << "," << packetType << "," << simTime()  << std::endl; // Added later by S. M. Sabit Bananee
    ///////////////////////////////////////////////////////////////
