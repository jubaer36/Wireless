...
#include "inet/common/ModuleAccess.h" // Added later by S. M. Sabit Bananee
...
std::ofstream Ieee80211YansErrorModel::logFileHeader;
std::ofstream Ieee80211YansErrorModel::logFileData;
bool Ieee80211YansErrorModel::logFileInitialized = false;

// Added later by S. M. Sabit Bananee
Ieee80211YansErrorModel::Ieee80211YansErrorModel()
{
    if (!logFileInitialized) {
        logFileHeader.open("HeaderErrorRate.csv", std::ios::out);  // Overwrite each run
        logFileHeader << "Simulation Time,Node,Header Length,Error rate,SNIR \n";

        logFileData.open("DataErrorRate.csv", std::ios::out);  // Overwrite each run
        logFileData << "Simulation Time,Node,Data Length,Error rate,SNIR\n";
...
Ieee80211YansErrorModel::~Ieee80211YansErrorModel()
{
    if (logFileHeader.is_open()) logFileHeader.close();
    if (logFileData.is_open()) logFileData.close();
}
////////////////////////////////////////////////
...
double Ieee80211YansErrorModel::getHeaderSuccessRate(const IIeee80211Mode* mode, unsigned int bitLength, double snr) const
{
    ...
    // Added later by S. M. Sabit Bananee
    logFileHeader << simTime() << "," << getContainingNode(this)->getFullName() << "," << bitLength << "," << 1-successRate << "," << snr << endl;
    logFileHeader.flush();
    ///////////////////////////////////////////
    return successRate;
}
...
double Ieee80211YansErrorModel::getDataSuccessRate(const IIeee80211Mode* mode, unsigned int bitLength, double snr) const
{
    ...
    // Added later by S. M. Sabit Bananee
    logFileData << simTime() << "," << getContainingNode(this)->getFullName() << "," << bitLength << "," << 1-successRate << "," << snr << endl;
    logFileData.flush();
    //////////////////////////////////////////
    return successRate;
}
