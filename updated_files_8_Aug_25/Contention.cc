...
simsignal_t Contention::cwUsedSignal = cComponent::registerSignal("cwUsed"); // Added later by S. M. Sabit Bananee
...
std::ofstream Contention::logFile;  // Added later by S. M. Sabit Bananee
bool Contention::logFileInitialized = false; // Added later by S. M. Sabit Bananee

void Contention::initialize(int stage)
{
    //Added later by S. M. Sabit Bananee
    if (!logFileInitialized) {
        logFile.open("cwUsed.csv", std::ios::out);  // Overwrite each run
        logFile << "Simulation Time,Node,Contetion Size\n";
...
void Contention::startContention(int cw, simtime_t ifs, simtime_t eifs, simtime_t slotTime, ICallback *callback)
{
    ...
    emit(cwUsedSignal, cw); //Added later by S. M. Sabit Bananee
    logFile << simTime() << "," << getContainingNode(this)->getFullName() << "," << cw << endl;
    // EV_INFO << "Halting simulation here: cw =" << cw << endl;
    // endSimulation();
