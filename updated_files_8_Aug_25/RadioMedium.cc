...
/*
#include "inet/common/packet/Packet.h" // Added later by S. M. Sabit Bananee
#include "inet/transportlayer/udp/UdpHeader_m.h" // Added later by S. M. Sabit Bananee
#include "inet/transportlayer/tcp_common/TcpHeader_m.h" // Added later by S. M. Sabit Bananee
#include "inet/networklayer/ipv4/IcmpHeader_m.h" // Added later by S. M. Sabit Bananee
#include "inet/networklayer/ipv4/Ipv4Header_m.h" // Added later by S. M. Sabit Bananee
#include "inet/linklayer/ieee80211/mac/Ieee80211Frame_m.h" // Added later by S. M. Sabit Bananee
*/
...
//std::ofstream RadioMedium::logFile;  // Added later by S. M. Sabit Bananee
//bool RadioMedium::logFileInitialized = false; // Added later by S. M. Sabit Bananee
...
void RadioMedium::initialize(int stage)
{
    ...
    if (!logFileInitialized) {
    	logFile.open("CollisionInfo.csv", std::ios::out);  // Overwrite each run
    	logFile << "Simulation Time,Receiver,Interferers\n";
...
const IInterference *RadioMedium::computeInterference(const IRadio *receiver, const IListening *listening) const
{
    ...
    /*// Added later by S. M. Sabit Bananee
    // Get the receiver module
    simtime_t startTime = noise->getStartTime();

    auto receiverModule = check_and_cast<const cModule *>(check_and_cast<const cObject *>(receiver));
    const char *receiverNodeName = getContainingNode(const_cast<cModule *>(receiverModule))->getFullName();

    if (!interferingReceptions->empty()) {
    	logFile << simTime() << "," << receiverNodeName << ",";
    	for (const IReception *reception : *interferingReceptions) {
    		const ITransmission *transmission = reception->getTransmission();
        	const IRadio *transmitterRadio = transmission->getTransmitter();
        	
        	auto transmitterModule = check_and_cast<const cModule *>(check_and_cast<const cObject *>(transmitterRadio));
        	const char *transmitterNodeName = getContainingNode(const_cast<cModule *>(transmitterModule))->getFullName();
        	
        	// Get the packet
        	auto layeredTx = dynamic_cast<const LayeredTransmission*>(transmission);
        	std::string packetType = "UNKNOWN";
        	if (layeredTx) {
        		throw cRuntimeError("halted");
        	}
        
        	if (layeredTx != nullptr) {
        		const Packet *packet = layeredTx->getPacket();
        		Ptr<const inet::ieee80211::Ieee80211MacHeader> macHeader = packet->peekAtFront<inet::ieee80211::Ieee80211MacHeader>();
        		auto innerPacket = packet->peekDataAt(macHeader->getChunkLength());
        		auto ipv4Header = inet::dynamicPtrCast<const inet::Ipv4Header>(innerPacket);
        		if (ipv4Header != nullptr) {
        			const auto& ipv4Payload = packet->peekDataAt(ipv4Header->getChunkLength());
        			if (packet->peekAtFront<inet::UdpHeader>() != nullptr)
            				packetType = "UDP";
            			else if (packet->peekAtFront<inet::tcp::TcpHeader>() != nullptr)
            				packetType = "TCP";
            			else if (packet->peekAtFront<inet::IcmpHeader>() != nullptr)
            				packetType = "ICMP";
            			else if (packet->peekAtFront<inet::Ipv4Header>() != nullptr)
            				packetType = "IPv4";
        		}
        	}

        	logFile << transmitterNodeName << " ( Start Time: " << startTime << ")" << ",";
        	//logFile << transmitterNodeName << ",";
    	}
    	logFile << endl;
    }
...
const IInterference *RadioMedium::computeInterference(const IRadio *receiver, const IListening *listening, const ITransmission *transmission) const
{
    ...
    if (!interferingReceptions->empty()) {
    	...
    	for (const IReception *reception : *interferingReceptions) {
        	...
        	//logFile << transmitterNodeName << " (" << packetType << " packet)" << ",";
        	logFile << transmitterNodeName << ",";
    	}
    	logFile << endl;
    }
