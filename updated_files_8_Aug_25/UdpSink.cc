...
#include "inet/common/TimeTag_m.h" //Added later by S. M. Sabit Bananee
#include "inet/networklayer/common/L3AddressTag_m.h" //Added later by S. M. Sabit Bananee

namespace inet {

Define_Module(UdpSink);

std::ofstream UdpSink::logFile;
bool UdpSink::logFileInitialized = false;
...
void UdpSink::initialize(int stage)
{
    ...
    if (stage == INITSTAGE_LOCAL) {
        ...
        if (!logFileInitialized) {
        	logFile.open("udpPacketTransmissionInfo.csv", std::ios::out);  // Overwrite each run
        	logFile << "Simulation Time,Sender,Receiver,Packet ID,Delay\n";
...
void UdpSink::handleMessageWhenUp(cMessage *msg)
{
    if (msg->isSelfMessage()) {
        ...
        //Added later by S. M. Sabit Bananee
            ...
            auto l3AddressInd = packet->findTag<inet::L3AddressInd>();
            if (l3AddressInd != nullptr) {
                inet::L3Address srcAddress = l3AddressInd->getSrcAddress();
                inet::L3Address destAddress = l3AddressInd->getDestAddress();
                simtime_t delay = msg->getArrivalTime() - msg->getCreationTime();
                logFile << simTime() << "," << srcAddress << "," << destAddress << " (" << getContainingNode(this)->getFullName() << ")" << "," << msg->getId() << "," << delay << endl;
            }
        ///////////////////////////
